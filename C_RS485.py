...
""" ################################################ """
""" Global variables """
RS485_ADDR_Teensy = 17 # RS485 slave address, communication to Teensy, address 17
...
# Bytearray 0..24, 25 Byte receive from teensy
BYTES_TEENSY_TO_RASPBERRY = 25
ReadDataMotor = bytearray(BYTES_TEENSY_TO_RASPBERRY)

# Bytearray 0..33, 34 Byte send to teensy
BYTES_RASPBERRY_TO_TEENSY = 34
WriteDataMotor = bytearray(BYTES_RASPBERRY_TO_TEENSY)

# Write anything to send data, 33 Byte, Raspberry -> Teensy
WriteByte0 = RS485_ADDR_Teensy
WriteByte1 = 66
WriteByte2 = 67
WriteByte3 = 68
WriteByte4 = 69
...
while True:
    ...
    if line:
        ...
        # Check Address 17 RS485_ADDR_Teensy
        if line[0] == RS485_ADDR_Teensy:
            ...
            if BytesReceived == BYTES_TEENSY_TO_RASPBERRY:
                ...
                and line[BYTES_TEENSY_TO_RASPBERRY-1] == CrcHByte:
                    ...
                    # Delaytime between two data packets, 5ms (read -> write) 
                    start = time.time()
                    end = start + 0.005
                    while time.time() <end:
                        pass
    ...
    # Timeout answer from slave
    if time.time() > Timeout +1.0:
        WriteData = True # Start next write data RS485
        print("timeout")
    ...
    if WriteData == True:
        ...
        WriteDataMotor = [WriteByte0, WriteByte1, WriteByte2, WriteByte3, WriteByte4,
                          ...
                          WriteByte30, WriteByte31, WriteByte32, WriteByte33]
        ...
        WriteDataMotor = [WriteByte0, WriteByte1, WriteByte2, WriteByte3, WriteByte4,
                          ...
                          WriteByte20, WriteByte21, WriteByte22, WriteByte23, WriteByte24,
                          WriteByte25, WriteByte26, WriteByte27, WriteByte28, WriteByte29,
                          WriteByte30, WriteByte31, CrcLByte, CrcHByte]
        ...
        # Delaytime output RS485 Transmitter enable, 5ms
        start = time.time()
        end = start + 0.005
        while time.time() <end:
            pass
